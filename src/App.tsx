...
interface AppUser {
  ...
  name: string;
  id: string;
  email: string; // ✅ FIX 3: always string, never undefined
}

// ── DEVELOPER ID ───────────────────────────────────────────
const DEV_EMAIL = 'shubhamvairagl0@gmail.com';

// ── CART VERSION for stale-data detection ──────────────────
const CART_VERSION = 'ab_cart_v1';
...
function Toast({ msg, type, onClose }:{ msg:string; type:'success'|'error'; onClose:()=>void }) {
  ...
  return (
    <motion.div
      ...
      style={{
        position:'fixed', bottom:'5.5rem', left:'50%', zIndex:9999,
        background:type==='error'?'#9B3D2A':'#2D4A2D',
        color:'#fff', padding:'12px 20px', borderRadius:8,
        fontSize:'0.82rem', fontFamily:'"Inter",sans-serif',
        boxShadow:'0 8px 32px rgba(0,0,0,0.3)',
        display:'flex', alignItems:'center', gap:8,
        whiteSpace:'nowrap', maxWidth:'90vw', fontWeight:500,
      }}>
...
function CartDrawer({ cart, onClose, onUpdateQty, onRemove, onClearCart, showToast, T, onBrowseWares }:
  { cart:CartItem[]; onClose:()=>void; onUpdateQty:(id:string,qty:number)=>void;
    ...
    showToast:(m:string,t?:'success'|'error')=>void; T:Tokens; onBrowseWares:()=>void }) {

  const total = cart.reduce((s,i) => s + i.product.price * i.qty, 0);
  const [showOrderForm, setShowOrderForm] = useState(false);
  const [showSuccess,   setShowSuccess]   = useState(false);
  // ✅ FIX 6: single orderId created once and passed through
  ...
  return (
    <>
      <motion.div
        ...
        <motion.div
          ...
          style={{
            ...
            width:'min(440px,100vw)', background:T.white,
            display:'flex', flexDirection:'column',
            boxShadow:`-12px 0 48px ${T.shadowL}`,
          }}>

          {/* Header */}
          <div style={{ padding:'20px 24px', borderBottom:`1px solid ${T.line}`, display:'flex',
          ...
          <div style={{ flex:1, overflowY:'auto', padding:'16px' }}>
            {cart.length === 0 ? (
              <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:'100%', gap:16, padding:'60px 20px' }}>
                ...
                <button onClick={onBrowseWares} style={{ padding:'10px 24px', background:T.forest, color:'#fff', border:'none', borderRadius:8, fontFamily:'"Inter",sans-serif', fontSize:'0.82rem', fontWeight:600, cursor:'pointer', display:'flex', alignItems:'center', gap:6 }}>
                  Browse Wares <ArrowRight size={14}/>
                </button>
              </div>
            ) : (
                  ...
                  <div key={item.product.id} style={{ background:T.offwhite, borderRadius:12, padding:'12px', display:'flex', gap:12, alignItems:'center', border:`1px solid ${T.line}` }}>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:8 }}>
                      <div style={{ display:'flex', alignItems:'center', background:T.paper, borderRadius:8, overflow:'hidden', border:`1px solid ${T.line}` }}>
                        <button
                          onClick={() => item.qty > 1 ? onUpdateQty(item.product.id, item.qty-1) : onRemove(item.product.id)}
                          style={{ width:30, height:30, border:'none', background:'transparent', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', color:T.inkM }}>
                          <Minus size={12}/>
                        </button>
                      ...
                      <button onClick={() => onRemove(item.product.id)} style={{ background:'none', border:'none', cursor:'pointer', color:T.rust, padding:4 }}>
                        <Trash2 size={14}/>
                      </button>
                    </div>
                  </div>
      ...
      {/* ✅ FIX 6: orderId flows from modal → success modal */}
      <AnimatePresence>
        {showOrderForm && (
          <OrderFormModal
            cart={cart}
...
// ✅ FIX 6: onSuccess now receives the orderId string
function OrderFormModal({ cart, onClose, onSuccess, T }:
  { cart:CartItem[]; onClose:()=>void; onSuccess:(orderId:string)=>void; T:Tokens }) {
  ...
  // ✅ FIX 6: single orderId, created once per modal instance
  const orderIdRef = useRef(`AB-${Date.now().toString(36).toUpperCase()}`);

  const inp: React.CSSProperties = {
    width:'100%', marginTop:6, padding:'11px 14px', background:T.offwhite,
    border:`1.5px solid ${T.line}`, borderRadius:8, fontSize:'0.9rem',
    outline:'none', color:T.ink, fontFamily:'"Inter",sans-serif', transition:'border-color .2s',
  };
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      // ✅ FIX 1: clean template variable names, no broken braces
      ...
      await emailjs.send(
        ...
        import.meta.env.VITE_EMAILJS_TEMPLATE_ID,
        {
          ...
          name:             form.name,       // From Name field in EmailJS
          to_email:         form.email,      // To Email field in EmailJS
          email:            form.email,      // Reply To field in EmailJS
          mobile:           form.mobile,
      ...
      onSuccess(orderId); // ✅ pass the same orderId upward
    } catch (e) {
      ...
      <motion.div
        ...
        <div style={{ padding:'20px 24px 16px', borderBottom:`1px solid ${T.line}`, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div>
            ...
            <p style={{ fontFamily:'"Inter",sans-serif', fontSize:'0.78rem', color:T.inkL, margin:0 }}>
              Order {orderIdRef.current} · ₹{total.toFixed(2)}
            </p>
          </div>
          ...
          <div style={{ background:T.paper, borderRadius:12, padding:'14px', marginBottom:20 }}>
            ...
            {cart.map(item => (
              <div key={item.product.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
                ...
                <p style={{ fontFamily:'"Inter",sans-serif', fontSize:'0.88rem', fontWeight:600, color:T.forest, margin:0 }}>
...
// ── ORDER SUCCESS ──────────────────────────────────────────
// ✅ FIX 6: removed unused `email` prop
function OrderSuccessModal({ orderId, onClose, T }:{ orderId:string; onClose:()=>void; T:Tokens }) {
  return (
    <motion.div
      initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
      ...
      <motion.div
        ...
        <motion.div
          ...
          style={{ width:72, height:72, background:T.forest, borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 20px', fontSize:'2rem', color:'#fff' }}>
          ✓
        </motion.div>
        <h2 style={{ fontFamily:'"Playfair Display",serif', fontSize:'1.5rem', fontWeight:700, color:T.ink, marginBottom:8 }}>Order Placed!</h2>
        <p style={{ fontFamily:'"Inter",sans-serif', fontSize:'0.88rem', color:T.inkL, marginBottom:20, lineHeight:1.6 }}>
          Your handcrafted items are on their way 🌿
        </p>
        ...
        <button
          onClick={onClose}
          style={{ width:'100%', padding:'13px', background:T.forest, color:'#fff', border:'none', borderRadius:10, fontFamily:'"Inter",sans-serif', fontSize:'0.88rem', fontWeight:600, cursor:'pointer' }}>
          Continue Shopping
        </button>
      </motion.div>
    </motion.div>
    ...
    <motion.div
      ...
      <motion.div
        ...
        <div style={{ marginBottom:20 }}>
          ...
          <input
            type="text" placeholder="e.g. Priya's Craft Studio"
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          <button
            onClick={() => shopName.trim() && onConfirm(shopName.trim())}
          ...
          <button onClick={onClose} style={{ padding:'12px', background:'transparent', color:T.inkL, border:`1px solid ${T.line}`, borderRadius:10, fontFamily:'"Inter",sans-serif', fontSize:'0.85rem', cursor:'pointer' }}>
            Cancel
          </button>
        </div>
      </motion.div>
...
function ImageUploader({ value, onChange, T }:{ value:string; onChange:(url:string)=>void; T:Tokens }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
      <input ref={fileRef}   type="file" accept="image/*"                  onChange={e=>{ const f=e.target.files?.[0]; if(f) void uploadFile(f); }} style={{ display:'none' }}/>
      <input ref={cameraRef} type="file" accept="image/*" capture="environment" onChange={e=>{ const f=e.target.files?.[0]; if(f) void uploadFile(f); }} style={{ display:'none' }}/>
      <input ref={folderRef} type="file" accept="image/*"                  onChange={e=>{ const f=e.target.files?.[0]; if(f) void uploadFile(f); }} style={{ display:'none' }}/>
            ...
            <>
              <div style={{ padding:'24px 20px 16px', textAlign:'center' }}>
                <div style={{ width:52, height:52, background:T.forestXL, borderRadius:12, display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 10px' }}>
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={T.forest} strokeWidth="2">
      ...
      <AnimatePresence>
        {urlMode && (
          <motion.div
            initial={{ opacity:0, y:-6 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-6 }} transition={{ duration:.15 }}
            ...
            <div style={{ display:'flex', gap:8 }}>
              <input
                type="url" autoFocus placeholder="https://example.com/photo.jpg"
                value={urlInput}
              ...
              <button onClick={() => { setUrlMode(false); setError(''); }} style={{ width:36, height:36, background:T.paper, border:`1px solid ${T.line}`, borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', color:T.inkL, flexShrink:0 }}>
                <X size={14}/>
              </button>
            </div>
...
function ReLoginModal({ email, shopName, onSuccess, onClose, T }:
  { email:string; shopName:string;
    onSuccess:(u:{ role:'buyer'|'seller'; name:string; id:string; email:string })=>void;
    onClose:()=>void; T:Tokens }) {
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          <button
            ...
            {loading
              ...
              : <>Activate Seller Account <ArrowRight size={15}/></>}
          </button>
          <button onClick={onClose} style={{ padding:'11px', background:'transparent', color:T.inkL, border:`1px solid ${T.line}`, borderRadius:10, fontFamily:'"Inter",sans-serif', fontSize:'0.85rem', cursor:'pointer' }}>
            Cancel
          </button>
        </div>
      </motion.div>
...
export function App() {
  ...
  // ✅ FIX 3: email is always a string
  const [user, setUser] = useState<AppUser>({ role:null, name:'', id:'', email:'' });

  const [toast,         setToast]         = useState<{ msg:string; type:'success'|'error' }|null>(null);
  const [showShopModal, setShowShopModal] = useState(false);
  const [reLoginData,   setReLoginData]   = useState<{ email:string; shopName:string }|null>(null);

  // ✅ FIX 5: versioned cart to avoid stale product data
  const [cart, setCart] = useState<CartItem[]>(() => {
    try {
      const ver  = localStorage.getItem('ab_cart_ver');
      const data = localStorage.getItem('ab_cart');
      if (ver === CART_VERSION && data) return JSON.parse(data) as CartItem[];
      return [];
    } catch { return []; }
  });
  ...
  // ✅ FIX 5: persist cart with version
  useEffect(() => {
    try {
      localStorage.setItem('ab_cart',     JSON.stringify(cart));
      localStorage.setItem('ab_cart_ver', CART_VERSION);
    } catch {}
  }, [cart]);

  // ✅ FIX 3: me() now saves email
  useEffect(() => {
    if (tokenStore.get()) {
      api.auth.me()
        .then(u => setUser({
          ...
          id:    u.id,
          email: u.email ?? '', // ← always populate email
        }))
        .catch(e => {
          if (e instanceof ApiError && e.status === 401) tokenStore.remove();
        });
    }
  }, []);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api.products
      .list({ category: selectedCat !== 'All' ? selectedCat : undefined, search: debSearch || undefined, limit:60 })
  ...
  const handleLogout = useCallback(() => {
    api.auth.logout();
    setUser({ role:null, name:'', id:'', email:'' });
    setView('home'); setSelProduct(null);
    showToast('Signed out');
  }, [showToast]);

  const handleLogin = useCallback((u: { role:'buyer'|'seller'; name:string; id:string; email?:string }) => {
    setUser({ role:u.role, name:u.name, id:u.id, email:u.email ?? '' });
    setView('home'); setSelProduct(null);
    showToast(`Welcome, ${u.name}!`);
  }, [showToast]);

  // ✅ FIX 2: guard empty email before proceeding
    ...
    <div style={{ minHeight:'100vh', background:T.offwhite, color:T.ink, fontFamily:'"Inter",sans-serif', paddingBottom:'5rem', position:'relative' }}>
      ...
      <div style={{ position:'relative', zIndex:1 }}>
        ...
        <AnimatePresence>{cartOpen && (
          <CartDrawer cart={cart} onClose={() => setCartOpen(false)} onUpdateQty={updateCartQty}
            onRemove={removeFromCart} onClearCart={() => setCart([])} showToast={showToast} T={T}
            onBrowseWares={() => { setCartOpen(false); setView('home'); setSelProduct(null); }}/>
        )}</AnimatePresence>
        <AnimatePresence>{showShopModal && (
          <ShopNameModal onConfirm={handleShopConfirm} onClose={() => setShowShopModal(false)} T={T}/>
        )}</AnimatePresence>
        <AnimatePresence>{reLoginData && (
          <ReLoginModal
        ...
        {/* ── HEADER ── */}
        <header style={{ position:'sticky', top:'env(safe-area-inset-top)', zIndex:100,
          background: darkMode ? 'rgba(26,26,26,0.97)' : 'rgba(255,255,255,0.95)',
          backdropFilter:'blur(12px)', borderBottom:`1px solid ${T.line}`, boxShadow:`0 1px 12px ${T.shadow}` }}>
          <div style={{ maxWidth:1200, margin:'0 auto', padding:'0 16px', height:60, display:'flex', alignItems:'center', gap:12 }}>
            {view === 'product' && (
              <button
                onClick={() => { setView(prevView); setSelProduct(null); }}
            ...
            <div style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:8 }}>
              ...
              {user.role ? (
                <div onClick={() => nav('profile')} style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 10px', background:T.paper, borderRadius:10, border:`1px solid ${T.line}`, cursor:'pointer' }}>
                  <div style={{ width:28, height:28, borderRadius:'50%', background:T.forest, display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontSize:'0.78rem', fontWeight:700, flexShrink:0 }}>
                ...
                <button onClick={() => nav('login')} style={{ padding:'8px 16px', background:T.forest, color:'#fff', border:'none', borderRadius:8, fontFamily:'"Inter",sans-serif', fontSize:'0.82rem', fontWeight:600, cursor:'pointer' }}>
                  Sign In
                </button>
              )}
            </div>
          ...
          <AnimatePresence mode="wait">
            {view === 'home' && (
              <motion.div key="home" initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }} transition={{ duration:.15 }}>
                <HomeView products={products} loading={loading} selectedCat={selectedCat}
                  setSelectedCat={setSelectedCat} onProduct={goToProduct} onAddToCart={addToCart} T={T}/>
              </motion.div>
            )}
            {view === 'sell' && (
            ...
            {view === 'product' && selProduct && (
              <motion.div key={`pd-${selProduct.id}`} initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }} transition={{ duration:.2 }}>
                <ProductDetailView product={selProduct} onProduct={goToProduct}
                  isOwn={selProduct.sellerId === user.id} onAddToCart={addToCart}
        ...
        <nav style={{ position:'fixed', bottom:0, left:0, right:0,
          background: darkMode ? 'rgba(26,26,26,0.97)' : 'rgba(255,255,255,0.97)',
          backdropFilter:'blur(12px)', borderTop:`1px solid ${T.line}`, zIndex:100,
          ...
          <div style={{ display:'flex', justifyContent:'space-around', maxWidth:480, margin:'0 auto', padding:'6px 0' }}>
            {([
              ...
              return (
                <button key={v}
                  onClick={() => v === 'cart' ? setCartOpen(true) : nav(v)}
                  ...
                  {v === 'cart' && cartCount > 0 && (
                    <span style={{ position:'absolute', top:2, right:6, background:T.rust, color:'#fff', borderRadius:'50%', width:16, height:16, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'0.55rem', fontWeight:700 }}>
                      {cartCount}
                    </span>
                  )}
                </button>
  ...
  return (
    <div>
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'24px 16px 40px' }}>
        ...
        {loading ? (
          <Spinner label="Discovering handcrafted wares…" T={T}/>
        ) : products.length === 0 ? (
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(2, 1fr)', gap:12 }} className="products-grid">
            ...
            {products.map((p, i) => (
              <motion.div
                key={p.id}
                initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay: Math.min(i * .04, .6) }}
...
function ProductDetailView({ product:init, onProduct, isOwn, onAddToCart, showToast, T }:
  { product:Product; onProduct:(p:Product)=>void; isOwn:boolean;
    onAddToCart:(p:Product,qty:number)=>void;
    showToast:(m:string,t?:'success'|'error')=>void; T:Tokens }) {
  ...
  useEffect(() => {
    Promise.all([
      api.products.get(product.id) as Promise<ProductDetail>,
      api.reviews.forProduct(product.id) as Promise<ReviewsResponse>,
    ])
      ...
      <div className="product-split" style={{ display:'grid', gridTemplateColumns:'50% 50%', minHeight:'70vh', maxWidth:1100, margin:'0 auto' }}>
        ...
        <div style={{ padding:'28px 24px', display:'flex', flexDirection:'column', gap:20 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:14, padding:'14px 16px', background:T.white, borderRadius:14, border:`1px solid ${T.line}` }}>
            <div style={{ width:44, height:44, borderRadius:'50%', background:T.forest, display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontFamily:'"Playfair Display",serif', fontSize:'1.1rem', fontWeight:700, flexShrink:0 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10, padding:'14px 16px', background:T.offwhite, borderRadius:14, border:`1px solid ${T.line}` }}>
            {([
              [<Truck size={14}/>, 'Free delivery on orders above ₹50'],
              [<Clock size={14}/>, 'Ships in 3–5 business days'],
              [<Shield size={14}/>, '7-day easy returns'],
          ...
          {!isOwn ? (
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              <button
                onClick={() => { setCartDone(true); setTimeout(() => setCartDone(false), 2000); onAddToCart(product, qty); }}
              ...
              <button
                onClick={() => { onAddToCart(product, qty); showToast('Added to basket!'); }}
            ...
            <div style={{ padding:'14px', background:T.forestXL, borderRadius:12, textAlign:'center', fontFamily:'"Inter",sans-serif', fontSize:'0.82rem', color:T.forest, fontWeight:600 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10 }}>
            {[
              { icon:<Shield size={18}/>, t:'Secure', d:'SSL checkout' },
              { icon:<RefreshCw size={18}/>, t:'Returns', d:'7 days' },
              { icon:<Package size={18}/>, t:'Gift Wrap', d:'Free' },
            ].map(({ icon, t, d }) => (
              ...
              {reviews.map(r => (
                <div key={r.id} style={{ padding:'18px 20px', background:T.white, borderRadius:14, border:`1px solid ${T.line}` }}>
                  <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:10, flexWrap:'wrap' }}>
                    <div style={{ width:36, height:36, borderRadius:'50%', background:T.forest, display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontFamily:'"Inter",sans-serif', fontSize:'0.8rem', fontWeight:700 }}>
                      {r.buyerName[0]}
                    </div>
                    <div>
                    ...
                    <span style={{ marginLeft:'auto', fontFamily:'"Inter",sans-serif', fontSize:'0.72rem', color:T.inkL }}>
                      {new Date(r.createdAt).toLocaleDateString('en-IN', { year:'numeric', month:'short', day:'numeric' })}
                    </span>
                  </div>
                  {r.comment && (
                    <p style={{ fontFamily:'"Crimson Pro",serif', fontSize:'0.95rem', color:T.inkM, lineHeight:1.7, fontStyle:'italic' }}>"{r.comment}"</p>
                  )}
                </div>
              ))}
            </div>
      ...
      {related.length > 0 && (
        <div style={{ padding:'32px 24px 48px', borderTop:`1px solid ${T.line}`, background:T.offwhite }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))', gap:16 }}>
            {related.map(p => (
              <div key={p.id} className="card-hover" onClick={() => onProduct(p)}
                style={{ background:T.white, borderRadius:14, overflow:'hidden', cursor:'pointer', boxShadow:`0 2px 10px ${T.shadow}`, border:`1px solid ${T.line}` }}>
...
function SellView({ onAdd, T }:{ onAdd:(p:Partial<Product>)=>Promise<void>; T:Tokens }) {
  ...
  const inp: React.CSSProperties = {
    width:'100%', marginTop:6, padding:'11px 14px', background:T.offwhite,
    border:`1.5px solid ${T.line}`, borderRadius:10, fontSize:'0.9rem',
    outline:'none', color:T.ink, fontFamily:'"Inter",sans-serif', transition:'border-color .2s',
  };
  const lbl: React.CSSProperties = {
    fontFamily:'"Inter",sans-serif', fontSize:'0.72rem', fontWeight:600,
  ...
  const handle = async () => {
    ...
    setSubmitting(true);
    try { await onAdd({ ...f, price: Number(f.price) }); }
  ...
  return (
    <div>
      <div style={{ textAlign:'center', marginBottom:32 }}>
        <div style={{ width:56, height:56, background:T.forest, borderRadius:16, display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 16px' }}>
      ...
      <div style={{ background:T.white, borderRadius:20, padding:'28px', boxShadow:`0 4px 24px ${T.shadow}`, border:`1px solid ${T.line}` }}>
        <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
          <div>
            ...
            <input type="text" placeholder="e.g. Hand-carved Wooden Bowl" value={f.name}
              onChange={e => setF({ ...f, name:e.target.value })} style={inp}/>
          </div>
          <div>
            <label style={lbl}>Product Photo *</label>
            <div style={{ marginTop:8 }}>
              <ImageUploader value={f.imageUrl} onChange={url => setF({ ...f, imageUrl:url })} T={T}/>
            </div>
          </div>
          <div className="sell-grid" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
            <div>
              <label style={lbl}>Price (₹) *</label>
              <input type="number" placeholder="0.00" value={f.price}
                onChange={e => setF({ ...f, price:e.target.value })} style={inp}/>
            </div>
            <div>
          ...
          <div>
            ...
            <textarea rows={4} placeholder="Tell buyers about your craft, materials used, size, etc."
              value={f.description} onChange={e => setF({ ...f, description:e.target.value })}
              style={{ ...inp, resize:'vertical' }}/>
          </div>
          <button
            onClick={() => void handle()}
            disabled={submitting || !f.imageUrl || !f.name || !f.price}
...
// ── PROFILE VIEW ───────────────────────────────────────────
function ProfileView({ user, onProduct, onLogout, T }:
  { user:AppUser; onProduct:(p:Product)=>void; onLogout:()=>void; T:Tokens }) {
  ...
  const handleDelete = async (pid: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      setMyProducts(p => p.filter(x => x.id !== pid));
    } catch {}
    ...
    <div style={{ maxWidth:960, margin:'0 auto' }}>
      ...
      <div style={{ background:T.white, borderRadius:20, padding:'28px', marginBottom:28, boxShadow:`0 4px 24px ${T.shadow}`, border:`1px solid ${T.line}`, display:'flex', alignItems:'center', gap:20, flexWrap:'wrap' }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          {user.email && (
            <p style={{ fontFamily:'"Inter",sans-serif', fontSize:'0.75rem', color:T.inkL, marginBottom:4 }}>{user.email}</p>
          )}
        ...
        <button onClick={onLogout} style={{ padding:'10px 20px', background:T.paper, color:T.inkM, border:`1px solid ${T.line}`, borderRadius:10, fontFamily:'"Inter",sans-serif', fontSize:'0.82rem', fontWeight:600, cursor:'pointer', flexShrink:0 }}>
          Sign Out
        </button>
      </div>
                    ...
                    <div style={{ position:'relative' }}>
                      ...
                      {deletingId === p.id && (
                        <div style={{ position:'absolute', inset:0, background:'rgba(255,255,255,0.85)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                          <Loader2 size={24} color={T.rust} style={{ animation:'spin 1s linear infinite' }}/>
                        </div>
                      )}
                    </div>
                  ...
                  <div style={{ padding:'0 12px 12px' }}>
                    ...
                    {confirmId === p.id && (
                      <button
                        ...
                        style={{ width:'100%', marginTop:4, padding:'6px', background:'transparent', border:`1px solid ${T.line}`, borderRadius:8, fontFamily:'"Inter",sans-serif', fontSize:'0.68rem', color:T.inkL, cursor:'pointer' }}>
                        Cancel
                      </button>
                    )}
                  </div>
...
function LoginView({ onLogin, T }:{ onLogin:(u:{ role:'buyer'|'seller'; name:string; id:string; email?:string })=>void; T:Tokens }) {
  ...
  const inp: React.CSSProperties = {
    width:'100%', marginTop:6, padding:'12px 14px', background:T.offwhite,
    border:`1.5px solid ${T.line}`, borderRadius:10, fontSize:'0.92rem',
    outline:'none', color:T.ink, fontFamily:'"Inter",sans-serif', transition:'border-color .2s',
  };
  const lbl: React.CSSProperties = {
    fontFamily:'"Inter",sans-serif', fontSize:'0.75rem', fontWeight:600, color:T.inkL,
  };
  ...
  const submit = async () => {
    ...
    try {
      const res = isReg
        ? await api.auth.register({ name, email, password:pass, role:'buyer' })
  ...
  return (
    <div style={{ minHeight:'80vh', display:'flex', alignItems:'center', justifyContent:'center', padding:'32px 16px' }}>
      <div style={{ width:'100%', maxWidth:420 }}>
        ...
        {!isReg && (
          <div style={{ marginBottom:20 }}>
            <p style={{ fontFamily:'"Inter",sans-serif', fontSize:'0.72rem', fontWeight:700, color:T.inkL, textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:10, textAlign:'center' }}>
              Quick Access — Demo Accounts
            </p>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
              {DEMO_ACCOUNTS.map(acc => (
                <button key={acc.label}
                  onClick={() => void doLogin(acc.email, acc.password, acc.label)}
        ...
        <div style={{ background:T.white, borderRadius:20, padding:'28px', boxShadow:`0 8px 40px ${T.shadowM}`, border:`1px solid ${T.line}` }}>
          ...
          <button onClick={() => void submit()} disabled={loading}
            ...
            {loading
              ...
              : <>{isReg ? 'Create Account' : 'Sign In'} <ArrowRight size={16}/></>}
          </button>
          ...
          <p style={{ textAlign:'center', marginTop:16, fontFamily:'"Inter",sans-serif', fontSize:'0.85rem', color:T.inkL }}>
            ...
            <button onClick={() => { setIsReg(r => !r); setError(''); }}
              style={{ background:'none', border:'none', color:T.forest, cursor:'pointer', fontWeight:600, fontSize:'0.85rem', fontFamily:'"Inter",sans-serif' }}>
  ...
  return (
    <div style={{ maxWidth:960, margin:'0 auto' }}>
      ...
      <div style={{ position:'relative', marginBottom:20 }}>
        ...
        <input type="text" placeholder="Search artisan by name…" value={searchSeller}
          ...
          {sellers.map(seller => {
            ...
            return (
              <div key={seller.sellerId} style={{ background:T.white, borderRadius:16, border:`1px solid ${T.line}`, overflow:'hidden', boxShadow:`0 2px 10px ${T.shadow}` }}>
                <button
                  onClick={() => setExpandedSeller(isOpen ? null : seller.sellerId)}
                ...
                <AnimatePresence>
                  {isOpen && (
                    <motion.div
                      initial={{ height:0, opacity:0 }} animate={{ height:'auto', opacity:1 }} exit={{ height:0, opacity:0 }} transition={{ duration:.2 }}
                              ...
                              <div style={{ padding:'10px 12px' }}>
                                ...
                                {confirmId === p.id ? (
                                  <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                                    ...
                                    <button onClick={e => void handleRemove(p.id, e)} disabled={!!deletingId}
